...
At the time of writing gnuplot contains a buggy {\tt tikz} terminal (it doesn't work with \CONTEXT) and no {\tt context} terminal. This may change in future, but at the moment you have to compile gnuplot yourself. The latest version of {\tt context.trm} can be found at
...
set term context { default }
                 ...
                 { mitered | [[\bf rounded]] | beveled }
                 { butt | [[\bf round]] | squared }
                 { [[\bf dashed]] | solid }
                 { dashlength | dl <DL> }
                 { linewidth | lw <LW> }
                 { fontscale <fontscale> }
...
\placefigure{An example of using png terminal}{\framed{\useGNUPLOTgraphic[bitmap][scale=700]}}
